...
exports.setActiveModule = function(activeModule) {
	...
	s.graph.edges().forEach(function(e) {
		if (e.targetModuleUid === activeModule) e.color = "#ff0000";
		else if (e.sourceModuleUid === activeModule) e.color = "#00aa00";
		else e.color = "#aaaaaa";
	});
	s.refresh();
};
...
exports.setActiveChunk = function(activeChunk) {
	...
	s.graph.nodes().forEach(function(n) {
		var m = n.module;
		if (m.chunks.indexOf(activeChunk) >= 0) n.color = "#000000";
		else n.color = "#aaaaaa";
		n.active = false;
	});
	s.graph.edges().forEach(function(e) {
		...
		if (sc && tc) e.color = "#000000";
		else if (sc) e.color = "#00aa00";
		else if (tc) e.color = "#ff0000";
		else e.color = "#aaaaaa";
	});
	s.refresh();
};
